...
def specific_state(id):
    try:
        state = [storage.all('State')[str(id)]]
        name = "State: {}".format(state[0].name)
    except KeyError:
        state = []
        name = "Not found!"
    return render_template('9-states.html', states=state, name=name)


@app.route("/states")
def all_state():
    name = "States"
    state = [x for x in storage.all('State').values()]
